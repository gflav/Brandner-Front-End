...
(function($){
  ...
  $('#subForm input').focusin(function($evt) {
    $evt.preventDefault();
    var $this = $(this);
  ...
  $('#subForm input').focusout(function($evt) {
    $evt.preventDefault();
    var $this = $(this);
    var $parent = $this.closest('form');
    var $label = $parent.find('label');
  ...
  $('#subForm label').click(function($evt) {
    $evt.preventDefault();
    var $this = $(this);
  ...
  $('.form-see-through label').click(function($evt) {
    $evt.preventDefault();
    var $this = $(this);
    var $parent = $this.closest('li');
	$parent.addClass('active');
  ...
  $.fn.parallax = function(options) {
      ...
      return this.each( function() {
          ...
          var altyBgPosition = yBgPosition / 2;
		  if(altyBgPosition < -110){ altyBgPosition = -110; }
               // Apply the Y Background Position to Set the Parallax Effect
              $this.css('background-position', 'left ' + altyBgPosition + 'px');			  
          }); // end of doc ready
		} //end of window width check
 	}); //end of scroll
	  
  }
  var whitepageBg = $(".page-template-page-white #site-content");
  if(whitepageBg) {
	  $(whitepageBg).parallax({
	      speed : 0.5
	  });	
  }
  ...
  var $search = {
    ...
    execute: function($options) {
      ...
      this.instance = $.ajax({
        ...
        data: $data,
        beforeSend: function(){
          $('.search-form-container').addClass('loading');
        ...
        if ($dom && $dom.length > 0) {
          ...
          if ($results && $results.length > 0) {
            ...
            $('.search-form-container').removeClass('loading');
          }
        }
      });
...
jQuery(document).ready(function(){
  var popup = jQuery('.quote-form-hidden');
  if(jQuery('.bd_product-template-default').length > 0){
    jQuery('a.btn.btn-quote').click(function(e){
      e.preventDefault();
      jQuery('.quote-form-hidden').addClass('open');
      jQuery('.quote-form-hidden .image .img').attr('src',jQuery('.product-gallery-container .product-image:first-child').find('img').attr('src'));
      jQuery('.quote-form-hidden .image').siblings('h4').html(jQuery('.post-content h1').html());
      jQuery('.quote-form-hidden .image').siblings('.product-summary').html(jQuery('.post-content').html());
      popup.show();
    });
    jQuery('.close-quote').click(function(e){
      e.preventDefault();
      popup.fadeOut();
      jQuery('.quote-form-hidden').removeClass('open');
    });
    jQuery('.quote-form-hidden textarea').html('Hi I&#39;d like a quote on '+jQuery('.post-content h1').html());
  }
});